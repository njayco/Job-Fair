...
router.post('/', async (req, res) => {
  try {
    const { job_description, job_url, cv_content } = req.body;
    ...
    if (!cv_content) {
      return res.status(400).json({ error: 'cv_content is required. Paste your CV in markdown format.' });
    }

    if (job_description.trim().length < 100) {
      return res.status(400).json({
        error: 'job_description seems too short. Please paste the full job description (at least 100 characters).',
      });
    }

    // Run the evaluation via Anthropic API
